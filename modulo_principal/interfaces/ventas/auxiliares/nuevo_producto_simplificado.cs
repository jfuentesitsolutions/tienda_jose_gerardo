                ...
                sp = new conexiones_BD.clases.sucursales_productos(
                    ...
                    using(espera_datos.splash_espera es=new espera_datos.splash_espera())
                    {
                        ...
                        if (es.ShowDialog() == DialogResult.OK)
                        {
                            if(es.Resultado>1)
                            {
                                MessageBox.Show("El producto se ingreso " + es.Resultado, "Exíto", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                ingresado = true;
                                this.Close();
                            }else if (es.Resultado < 0 || es.Resultado==1)
                            {
                                MessageBox.Show("El producto no se pudo ingresar, revise la conexión al servidor", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                            }
