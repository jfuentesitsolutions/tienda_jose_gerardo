...
namespace interfaces.mantenimientos
{
    public partial class marcas : Form
    {
        ...
        {
            tabla = new utilitarios.cargar_tablas(tablaMarcas, txtBuscar, conexiones_BD.clases.marcas.datosTabla(false), "nombre", lblReg);
            tabla.cargar();
        }
        ...
        private void modificarMarca()
        {
            if (!validar())
            {
                if (!validarExistencias())
                {
                    ...
                    if (marca.modificar(true, false) > 0)
                    {
                        habilitar(false);
                        vaciarDatos();
                        cargarTablas();
            ...
            if (idmarca != null)
                {
                    ...
                    if (marca.eliminar(true, false) > 0)
                    {
                        habilitar(false);
                        vaciarDatos();
                        cargarTablas();
