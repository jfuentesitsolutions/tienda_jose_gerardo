...
namespace interfaces.mantenimientos
{
    public partial class clientes : Form
    {
        ...
        private bool validarExistencias()
        {
            ...
            if (modificar)
            {
                if (cliente.validarCamposcondicorANDActu(true, 1, false) > 1)
                {
                    existe = true;
                }
            }
            ...
            {
                if (cliente.validarCamposcondicorAND(true, false) > 0)
                {
                    existe = true;
                }
            }
        ...
        private void guardar()
        {
            if (!validar())
            {
                if (!validarExistencias())
                {
                    ...
                    if (cliente.guardar(true, false) > 0)
                    {
                        vaciarDatos();
                        cargarTablas();
                        if (nuevoClientes)
            ...
            if (!validar())
            {
                if (!validarExistencias())
                {
                    ...
                    cliente = new conexiones_BD.clases.clientes(
                        ...
                        this.estado());
                    if (cliente.modificar(true, false) > 0)
                    {
                        habilitar(false);
                        vaciarDatos();
                        cargarTablas();
        ...
        private void eliminar()
        {
            if (idcliente != null)
            {
                if (MessageBox.Show("¿Desea eliminar este registro?", "Pregunta", MessageBoxButtons.YesNo, MessageBoxIcon.Asterisk) == DialogResult.Yes)
                {
                    ...
                    if (cliente.eliminar(true, false) > 0)
                    {
                        habilitar(false);
                        vaciarDatos();
                        cargarTablas();
