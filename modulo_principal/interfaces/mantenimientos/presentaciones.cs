...
namespace interfaces.mantenimientos
{
    public partial class presentaciones : Form
    {
        ...
        {
            tabla = new utilitarios.cargar_tablas(tablaPresentaciones, txtBuscar, conexiones_BD.clases.presentaciones.datosTabla(false), "nombre_presentacion", lblReg);
            tabla.cargar();
        }
        ...
        private void guardar()
        {
            if (!validar())
            {
                if (!validarExistencias())
                {
                    ...
                    if (presentacion.guardar(true, false) > 0)
                    {
                        vaciarDatos();
                        cargarTablas();
        ...
        private void modifica()
        {
            if (!validar())
            {
                if (!validarExistencias())
                {
                    ...
                    if (presentacion.modificar(true, false) > 0)
                    {
                        habilitar(false);
                        vaciarDatos();
                        cargarTablas();
        ...
        private void eliminar()
        {
            if (idpresentacion != null)
            {
                ...
                if (presentacion.eliminar(true, false) > 0)
                {
                    habilitar(false);
                    vaciarDatos();
                    cargarTablas();
