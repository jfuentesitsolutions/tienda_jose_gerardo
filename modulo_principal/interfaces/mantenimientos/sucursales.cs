...
namespace interfaces.mantenimientos
{
    public partial class sucursales : Form
    {
        ...
        {
            tabla = new utilitarios.cargar_tablas(tablaSucursales, txtBuscar, conexiones_BD.clases.sucursales.datosTabla(false), "numero_de_sucursal", lblRegistros);
            tabla.cargar();
        }
        ...
        private void guardarSucursal()
        {
            if (!validar())
            {
                if (!validandoExistencias())
                {
                    ...
                    if (sucursal.guardar(true, false) > 0)
                    {
        ...
        private void modificarSucursal()
        {
            if (!validar())
            {
                ...
                if (sucursal.modificar(true, false) > 0)
                {
                    vaciarDatos();
                    habilitar(false);
                    cargarTabla();
        ...
        private void eliminarSucursal()
        {
            if (idsucursal!=null)
            {
                ...
                if (sucursal.eliminar(true, false) > 0)
                {
                    vaciarDatos();
                    habilitar(false);
                    cargarTabla();
