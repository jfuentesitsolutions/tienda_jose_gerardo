    ...
    public partial class utilidades : Form
    {
        ...
        private void guardarUtilidad()
        {
            if (!validar())
            {
                if (!validarExistencias())
                {
                    ...
                    if (utilidad.guardar(true, false) > 0)
                    {
                        vaciarDatos();
                        cargarTablas();
                        txtUtilidad.Focus();
        ...
        private void modificarUtilidad()
        {
            if (!validar())
            {
                if (!validarExistencias())
                {
                    ...
                    if (utilidad.modificar(true, false) > 0)
                    {
                        habilitar(false);
                        vaciarDatos();
                        cargarTablas();
        ...
        private void eliminarUtilidad()
        {
            if (utilidad != null)
            {
                ...
                if (utilidad.eliminar(true, false) > 0)
                {
                    habilitar(false);
                    vaciarDatos();
                    cargarTablas();
        ...
        {
            utilitarios.cargandoListas.cargarLista(conexiones_BD.clases.tipo_compra.datosTabla(false),
                listaTipoCompra,
                "nombre",
                "idtipo_compra");
        }
