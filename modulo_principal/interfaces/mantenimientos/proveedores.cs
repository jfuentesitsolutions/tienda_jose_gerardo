...
namespace interfaces.mantenimientos
{
    public partial class proveedores : Form
    {
        ...
        {
            utilitarios.cargandoListas.cargarTexbox(conexiones_BD.clases.proveedores.datosTabla(false), txtProveedor);
        }
            ...
            if (!validar())
            {
                if (!validarExistencias())
                {
                    proveedor = new conexiones_BD.clases.proveedores(idproveedor, txtCodigo.Text, txtProveedor.Text,
                        txtDui.Text, txtNit.Text, txtNcr.Text, txtDireccion.Text, txtTelefono.Text, txtCorreo.Text);
                    if (proveedor.modificar(true, false) > 0)
                    {
                        habilitar(false);
                        vaciarDatos();
                        cargarTablas();
        ...
        private void eliminar()
        {
            if (idproveedor != null)
            {
                ...
                if (proveedor.eliminar(true, false) > 0)
                {
