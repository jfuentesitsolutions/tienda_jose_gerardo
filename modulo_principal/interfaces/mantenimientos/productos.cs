...
namespace interfaces.mantenimientos
{
    public partial class productos : Form
    {
        ...
        private bool ingreso_nuevo = false;
        ...
        public bool Ingreso_nuevo { get => ingreso_nuevo; set => ingreso_nuevo = value; }
        ...
        private void cargarTablas()
        {
            ...
            if (!ingreso_nuevo)
            {
                ...
                tabla2 = new utilitarios.cargar_tablas(tabla_presentacion, txtBuscarP, conexiones_BD.clases.presentaciones.datosTabla(false), "nombre_presentacion");
            ...
            if (modificar)
            {
                try
                {
                    if (MessageBox.Show("¿Deseas eliminar esta presentación?", "Pregunta", MessageBoxButtons.YesNo, MessageBoxIcon.Asterisk) == DialogResult.Yes)
                    {
                        ...
                        if (prpr.eliminar(false, false) > 0)
                        {
                            cargarTablaPresentaciones();
                        }
                    }
        ...
        {
            utilitarios.cargandoListas.cargarLista(conexiones_BD.clases.utilidades.datosTablaMayoreo(false),
                listaMayoreo, "nombre", "idutilidad_compra");
            utilitarios.cargandoListas.cargarLista(conexiones_BD.clases.utilidades.datosTablaDetalle(false),
                listaUtilidadDetalle, "nombre", "idutilidad_compra");
            utilitarios.cargandoListas.cargarLista(conexiones_BD.clases.marcas.datosTabla(false),
                listaMarca, "nombre", "idmarca");
            utilitarios.cargandoListas.cargarLista(conexiones_BD.clases.categorias.datosTabla(false),
                listaCategoria, "nombre_categoria", "idcategoria");
            utilitarios.cargandoListas.cargarLista(conexiones_BD.clases.estantes.datosTabla(false),
                listaEstante, "nombre", "idestante");

            if (sesion.Datos[3].Equals("Administradores"))
            {
                utilitarios.cargandoListas.cargarLista(conexiones_BD.clases.sucursales.datosTabla(false),
        ...
        private void guarda()
        {
            ...
            if (!validar())
            {
                if (!validarExistencias())
                {
                    ...
                    {
                        es.Funcion_sentencia = guardando_productos;
                        es.Tipo_operacion = 2;
        ...
        private int guardando_productos()
        {
            op = new conexiones_BD.operaciones();
            op.Conexion_remota = conexion_remota;
            return op.transaccionProductos_Presentaciones_Proveedores(generaProveedores(), generaPresentaciones(), pro, sp, codi);
        }
        ...
        {
            utilitarios.cargandoListas.cargarLista(conexiones_BD.clases.codigos.cargarCodigos(this.idproducto, false),
